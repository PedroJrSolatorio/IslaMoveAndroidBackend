  ...
  async () => {
    ...
    try {
      ...
      for (const userDoc of usersSnapshot.docs) {
        ...
        if (userData.userType === "DRIVER" && userData.driverData?.documents) {
          const driverDocuments = userData.driverData.documents;
          const updatedDocuments = {};

          for (const [docType, doc] of Object.entries(driverDocuments)) {
            if (
              doc.expiryDate &&
              doc.expiryDate < now &&
              doc.status !== "EXPIRED"
              ...
              hasUpdates = true;
              console.log(`Marking ${docType} as expired for driver ${userId}`);
            } else {
              updatedDocuments[docType] = doc;
            }
          }

          if (hasUpdates) {
            updates["driverData.documents"] = updatedDocuments;
          }
        }

        // Check passenger student document
        if (userData.studentDocument) {
          if (
            userData.studentDocument.expiryDate &&
            userData.studentDocument.expiryDate < now &&
            userData.studentDocument.status !== "EXPIRED"
          ) {
            updates.studentDocument = {
            ...
            hasUpdates = true;
            console.log(
              `Marking student document as expired for user ${userId}`
            );
          }
        }
      ...
      console.log(
        `✅ Expired documents check complete. Updated ${updatedCount} user(s)`
      );
    } catch (error) {
      console.error("Error checking expired documents:", error);
      await sendAlertEmail(
        "ERROR: Expired Documents Check Failed",
        `The scheduled expired documents check encountered an error:
